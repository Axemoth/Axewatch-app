...
class AxewatchViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        val idMemory = application.getSharedPreferences("axewatch_id_memory", android.content.Context.MODE_PRIVATE)
        repository = AxewatchRepository(db, idMemory)
    ...
    val portfolioSummary: StateFlow<PortfolioSummary> = combine(holdings, stocks) { hList, sList ->
        ...
        PortfolioSummary(
            ...
            // No prev-close feed per holding yet — unknown, not zero.
            dayReturn = null,
            dayReturnPercent = null,
            // XIRR needs dated cashflows (holdings carry no purchase dates yet).
            xirrPercent = null,
        ...
        PortfolioSummary(0.0, 0.0, 0.0, 0.0, null, null, null, 0)
    )
    ...
    // On-device trade-idea scan (6h cache inside the repository). Nothing
    // calls this automatically — the Model Signals tab triggers it.
    fun scanIdeas(force: Boolean = false) {
        viewModelScope.launch {
            _isRefreshing.value = true
            try {
                val ideas = repository.scanTradeIdeas(force)
                if (ideas.isEmpty()) {
                    _toastMessage.emit("Scan complete — no qualifying setups right now")
                }
            } finally {
                _isRefreshing.value = false
            }
    ...
    fun checkAllotment(pan: String, ipoSymbol: String, holderName: String = "Self") {
        viewModelScope.launch {
            ...
            _toastMessage.emit(
                when (record.status) {
                    "ALLOTTED" -> "Congratulations! ${record.sharesAllotted} shares ALLOTTED for ${record.ipoSymbol}"
