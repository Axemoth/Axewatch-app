package com.aistudio.axewatch.trader.data.analysis
...
object TechnicalAnalysisEngine {
    ...
    fun computeOutlook(
        ...
        // Historical passage days (direction-aware: up-targets for BUY-side,
        // down-targets for SELL-side)
        val target1Pct = abs(target1 / currentPrice - 1.0) * 100.0
        val estDays = firstPassageDays(candles, target1Pct, isBuy)
        ...
        return TradeOutlook(
            ...
            // No on-device walk-forward exists: null renders as "unvalidated".
            // Accuracy must come from a real validation run, never a formula.
    ...
    private fun firstPassageDays(candles: List<CandleBar>, targetPct: Double, isBuy: Boolean = true): Int {
        val n = candles.size
        val hitDays = mutableListOf<Int>()
        for (i in max(0, n - 40) until n - 6 step 2) {
            val entry = candles[i].close
            val upPx = entry * (1.0 + targetPct / 100.0)
            val downPx = entry * (1.0 - targetPct / 100.0)
            for (j in i + 1 until min(n, i + 30)) {
                val hit = if (isBuy) candles[j].high >= upPx else candles[j].low <= downPx
                if (hit) {
                    hitDays.add(j - i)
                    break
                }
    ...
    private fun fallbackOutlook(symbol: String): TradeOutlook {
        // Fewer than 15 candles: refuse to invent a signal, levels, accuracy
        // figures, news or reasons. The UI renders this as "insufficient data".
        return TradeOutlook(
            symbol = symbol,
            score = 0,
