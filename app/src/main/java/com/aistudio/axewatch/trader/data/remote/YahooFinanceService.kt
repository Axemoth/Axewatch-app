...
class YahooFinanceService {
    ...
    suspend fun fetchStockQuote(
        ...
        try {
            ...
            StockQuote(
                ...
                // Yahoo chart meta carries no fundamentals: null renders "—".
                peRatio = null,
                marketCapCr = null
            )
        } catch (e: Exception) {
    ...
    suspend fun fetchMarketIndex(
        ...
        try {
            ...
            MarketIndex(
                ...
                // Advance/decline needs constituent breadth, which a single
                // index quote cannot provide: 0 renders as "—".
                advances = 0,
                declines = 0
            )
        } catch (e: Exception) {
    ...
    suspend fun searchSymbols(query: String): List<Pair<String, String>> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val url = "$SEARCH_URL?q=$encoded&quotesCount=16&newsCount=0"
            val request = Request.Builder()
                .url(url)
                .header("User-Agent", USER_AGENT)
                .header("Accept", "application/json")
